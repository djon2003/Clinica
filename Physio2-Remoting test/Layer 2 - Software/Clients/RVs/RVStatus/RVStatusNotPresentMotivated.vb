...
Namespace Accounts.Clients.Folders
    Partial Public Class RVsStatus
        ...
        Private Class RVStatusNotPresentMotivated
            Inherits RVStatus
                ...
                Try
                    ...
                    If lockedFolder AndAlso status.extraInfos.noFTDefault <> 0 Then
                        Dim defaultText As String = status.extraInfos.textFTDefault.Replace(vbCrLf, "").Replace("'", "''") & _
                                                    ...
                                                    "Inscription à " & DateFormat.getTextDate(Date.Now) & " " & DateFormat.getTextDate(Date.Now, DateFormat.TextDateOptions.FullTime) & " d''une absence motivée datée le " & DateFormat.getTextDate(status.rv.dateHeure) & " à " & DateFormat.getTextDate(status.rv.dateHeure, DateFormat.TextDateOptions.ShortTime) & _
...
SkipModif:
                ...
                If PreferencesManager.getGeneralPreferences("AlertTRPOnRVAbsence") = "" OrElse PreferencesManager.getGeneralPreferences("AlertTRPOnRVAbsence") = True Then
                    ...
                    newAlert.message = "Le rendez-vous du " & DateFormat.getTextDate(status.rv.dateHeure) & " " & DateFormat.getTextDate(status.rv.dateHeure, DateFormat.TextDateOptions.ShortTime) & " au dossier #" & status.rv.noFolder & " du client " & status.rv.clientName & " est passé au statut absent motivé"
            ...
            Public Overrides Sub verifyChange(ByVal status As RVStatusChange)
                ...
                myNewRaison = myInputBoxPlus.Prompt("Veuillez entrer la raison de l'absence :", "Absence - Motivée")
                If myNewRaison = "" Then
