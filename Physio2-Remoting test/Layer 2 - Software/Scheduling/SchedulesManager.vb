...
#End Region
    ...
    Public Overrides Function addItemable(ByVal newItem As Schedule) As String
        ...
        Catch ex As Exception
            'Trying to find a null exception bug
            addErrorLog(New Exception("newItem is Nothing=" & (newItem Is Nothing) & ",fasterAccessor is nothing=" & (fasterAccessor Is Nothing), ex))
        End Try
    ...
    Private Overloads Sub load(ByVal noSchedule As Integer)
        ...
        Else
            For Each curRow As DataRow In data.Tables(0).Rows
                ...
                If Me.getItemable(horaire.noItemable) IsNot Nothing Then Continue For ' The schedule had been modified by another at the same time as loading
    ...
    Public Overrides Function getItemable(ByVal noItem As Integer) As Schedule
        Return MyBase.getItemable(noItem)
    End Function

    Public Function getSchedule(ByVal noUser As Integer, ByVal scheduleDate As Date) As Schedule
        If Not scheduleDate.Equals(LIMIT_DATE) Then scheduleDate = scheduleDate.AddDays(scheduleDate.DayOfWeek * -1)
        Dim key As String = noUser & "_" & DateFormat.getTextDate(scheduleDate)
        If fasterAccessor.ContainsKey(key) = False AndAlso scheduleDate = LIMIT_DATE Then Return Nothing
    ...
    Public Function getDefaultSchedule(ByVal noUser As Integer) As Schedule
        Return getSchedule(noUser, LIMIT_DATE)
    End Function
