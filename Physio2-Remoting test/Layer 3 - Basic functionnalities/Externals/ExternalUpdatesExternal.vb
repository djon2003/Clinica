Public Class ExternalUpdatesExternal
    ...
    Public Overrides Function canSoftwareBeUpdated(ByVal informUser As Boolean) As Boolean
        If Not TCPClient.getInstance().isConnected Then Return False

        Dim canUpdate As Boolean = sendCanUpdateCommand(True)
        
        If Not canUpdate AndAlso informUser Then
            MessageBox.Show("Impossible de mettre à jour car d'autres Clinica sont en cours d'exécution", "Impossible de mettre à jour", MessageBoxButtons.OK, MessageBoxIcon.Stop)
        End If

        Return canUpdate
    End Function

    Private Function sendCanUpdateCommand(ByVal isLocked As Boolean) As Boolean
        Dim updateLock As New TCPCommands.UpdateLock(TCPClient.getInstance(), isLocked)
        Dim canUpdate As TCPAnswers.CanUpdate = TCPHelper.getInstance().sendWaitCommunication(Nothing, updateLock, GetType(TCPAnswers.CanUpdate))

        Return canUpdate IsNot Nothing AndAlso canUpdate.canUpdate
    End Function

    Public Overrides Sub onEndSoftwareUpdate()
        sendCanUpdateCommand(False)
    End Sub
End Class
